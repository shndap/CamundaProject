package mci.camunda.project.Assigner;
...
public class Assigner {
    private final Document bpmn;
    private final boolean isOkay;

    public Assigner(BPMN bpmn) {
        this.bpmn = DocumentLoader.convertToDocument(bpmn.getXml());
        isOkay = bpmn.isOkay();
    }
    ...
    private boolean assignToID(String ID, Form form) {
        ...
        formNode.setAttribute("formKey", "camunda-forms:bpmn:" + form.getXMLID());
    ...
    private boolean hasForm(NodeList list, Form form) {
        for (int i = 0; i < list.getLength(); i++) {
            ...
            if (item.hasAttribute("id") && item.getAttribute("id").equals(form.getXMLID())) {
                return true;
            }
        }
